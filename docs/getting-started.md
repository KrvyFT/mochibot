...
其他人私聊 bot 时按**临时访客**接待：沿用主人的人格、Core、日记和记忆，但这段对话不写入主人记忆，重启后访客上下文会丢掉。群聊和非私聊仍只应主人。可在管理后台设置页关掉「允许 Telegram 访客」，或设 `TELEGRAM_ALLOW_VISITORS=false`。
...
设置页下方可以改时区、每天 Free Time 次数，以及：

- **睡眠时段**（默认 01:00–06:00）：这段时间说话也不会叫醒；过了结束点会自动醒。她醒了你还没开口时，Free Time 会变少，你发一条后恢复。
- **Free Time 活跃时段**（默认 08:00–00:30，可跨午夜）：主动找你的机会铺在这个区间里

改完大约一分钟内生效。当天已经排好的机会会按新窗口重排。
...
**电脑关了 = bot 离线**
MochiBot 运行在你的电脑上。电脑关机、合盖、断网，bot 就不会回消息。想让 bot 24 小时在线，可以部署到 Linux 服务器：如果希望主人在聊天里让 Mochi 自助更新，请使用官方 Git 安装并通过 `scripts/start.py` 运行；Docker 需要由宿主机负责更新镜像。详见 [README 的更新说明](../README.md#更新)。
...
**最省事的方法：** 直接对 Mochi 说「你更新一下」。Git 本地安装会检查 GitHub 最新正式 Release，先回复你，然后短暂离线，用正式版本覆盖程序代码并重启；`.env` 和 `data/` 不会被覆盖。她不会每天主动查询版本，也不会在 `look_around` 中混入更新信息。
...
这是正常的。MochiBot 的心跳大约每 30 秒检查一次。Free Time 主动消息排在配置的活跃时段里（默认 08:00–00:30）；默认 01:00–06:00 是睡眠时段，这段时间不会因为你说话而叫醒，过了结束点会自动醒。她醒了你还没开口时，主动找你会变少。这些时间都可以在管理后台的相处偏好里改。刚启动的话等一会儿就好。
