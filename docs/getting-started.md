...
- **睡眠时段**（默认 01:00–06:00）：这段时间说话也不会叫醒
...
这是正常的。MochiBot 的心跳大约每 30 秒检查一次。Free Time 主动消息排在配置的活跃时段里（默认 08:00–00:30）；默认 01:00–06:00 是睡眠时段，这段时间不会因为你说话而叫醒（过了叫醒点才会）。这些时间都可以在管理后台的相处偏好里改。刚启动的话等一会儿就好。
