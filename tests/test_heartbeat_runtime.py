...
def test_plan_date_before_0030_belongs_to_yesterday():
    now = datetime(2026, 9, 2, 0, 15, tzinfo=UTC)
    plan_date, start, end = free_time_plan_bounds(now)
    assert plan_date == "2026-09-01"
    assert start == datetime(2026, 9, 1, 8, 0, tzinfo=UTC)
    assert end == datetime(2026, 9, 2, 0, 30, tzinfo=UTC)
    assert in_free_time_window(now)


def test_plan_date_after_0030_is_today_and_gap_until_8am():
