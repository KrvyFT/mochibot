...
def test_legacy_periodic_attention_is_retired_without_a_run():
    ...
    assert get_schedulable_runs(now=due) == []
    conn = _connect()
    schedule = conn.execute(
        "SELECT 1 FROM heartbeat_schedules WHERE entry_kind = 'attention'"
    ).fetchone()
    conn.close()
    assert schedule is None


def test_changed_attention_fact_schedules_one_run():
    due = datetime(2026, 8, 24, 2, 0, tzinfo=timezone.utc)
    assert sync_attention_facts(
        "test",
    ...
    assert runs[0]["wake_reason"] == "observer_change"
    assert json.loads(runs[0]["facts_json"])[0]["stable_key"] == "new-fact"


def test_attention_defers_new_facts_behind_unfinished_run():
    first_due = datetime(2026, 8, 24, 2, 0, tzinfo=timezone.utc)
    sync_attention_facts(
        "test",
        [{"stable_key": "initial", "facts": {"value": "first"}}],
    ...
    assert json.loads(runs[0]["facts_json"])[0]["stable_key"] == "initial"
    conn = _connect()
...
def test_attention_does_not_schedule_unchanged_facts():
    first_due = datetime(2026, 8, 24, 2, 0, tzinfo=timezone.utc)
    sync_attention_facts(
        "test",
    ...
    assert not sync_attention_facts(
        "test",
        [{"stable_key": "same-fact", "facts": {"value": "same"}}],
        observed_at=second_due,
        freshness_seconds=7200,
...
def test_legacy_periodic_attention_backlog_is_retired():
    conn = _connect()
    ...
    conn = _connect()
    rows = conn.execute(
        "SELECT status, outcome FROM heartbeat_runs ORDER BY run_key"
    ).fetchall()
    conn.close()
    assert [dict(row) for row in rows] == [
        {"status": "delivered", "outcome": "retired"},
        {"status": "delivered", "outcome": "retired"},
        {"status": "delivered", "outcome": "retired"},
    ]
...
def test_oldest_attention_backoff_blocks_later_legacy_runs():
    ...
    conn.execute(
        ...
        "'observer_change', ?, 'pending', ?, '2026-08-24T01:00:00+00:00')",
        (facts_json, (now + timedelta(hours=1)).isoformat()),
    )
    conn.execute(
        "INSERT INTO heartbeat_runs "
        "(run_key, entry_kind, user_id, channel_id, transport, wake_reason, "
        ...
        "'observer_change', ?, 'pending', '2026-08-24T02:00:00+00:00')",
        (facts_json,),
    )
    conn.commit()
    conn.close()
...
def test_claim_quarantines_malformed_attention_facts():
    ...
    conn.execute(
        ...
        "(run_key, entry_kind, user_id, channel_id, transport, wake_reason, "
        "facts_json, status, created_at) "
        "VALUES ('attention:deferred', 'attention', 1, 100, 'fake', "
        "'observer_change', '[]', 'deferred', ?)",
    ...
    deferred_status = conn.execute(
        "SELECT status FROM heartbeat_runs WHERE run_key = 'attention:deferred'"
    ).fetchone()["status"]
    conn.close()
    assert dict(row) == {
        "status": "delivered",
    ...
    assert deferred_status == "pending"
...
def test_deferred_attention_drops_resolved_facts_before_promotion():
    ...
    second_due = first_due + timedelta(minutes=10)
    sync_attention_facts(
        "test",
        [],
    ...
    assert materialize_due_runs(
        ...
        channel_id=100,
        transport="fake",
        now=second_due + timedelta(minutes=10),
        free_time_min_minutes=90,
        free_time_max_minutes=240,
    ) == []
    claimed = claim_run(first[0], now=second_due + timedelta(minutes=10))
    ...
    runs = get_schedulable_runs(now=second_due + timedelta(minutes=10))
    assert len(runs) == 1
...
def test_attention_promotes_one_coalesced_dirty_snapshot_after_terminal():
    ...
    sync_attention_facts(
        "test",
        [{"stable_key": "fact-0", "facts": {"value": 0}}],
        observed_at=first_due,
        freshness_seconds=7200,
    )
    first = materialize_due_runs(
    ...
    claimed = claim_run(first[0], now=first_due)
    assert claimed is not None
    ...
    promoted = get_schedulable_runs(now=third_due)
...
@pytest.mark.asyncio
async def test_free_time_reads_completed_interactions_without_active_chat_turns(
    mock_llm_factory,
):
    for number in range(3):
    ...
    save_message(
        1,
        "assistant",
        "already sent outreach",
        turn_id="free_time:prior",
        processed=True,
    )
    ...
    assert "already sent outreach" in base_context
    assert "already sent outreach" not in event
    assert "<autonomous_runtime_event>" in event
