...
有自己的心跳，会在默认的 08:00–00:30 均匀地主动找你（可在后台相处偏好里改），也会好奇你的一切。
...
- 调整每天主动消息（Free Time）的数量，例如「每天最多主动找我 5 次」。机会会铺满当前配置的活跃时段（默认 08:00–00:30），相邻至少 15 分钟，上限是这个窗口里 15 分钟一格能放下的最多次数。睡眠时段和活跃时段都可以在管理后台的相处偏好里改。
...
| 主动陪伴 | 不必总等你先开口，机会在配置的活跃时段（默认 08:00–00:30）均匀分布，次数不少于每天安排 |
