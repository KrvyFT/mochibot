...
Aggregates diagnostics from prompt, memory, database, and error modules.
...
def _check_prompt_size() -> dict:
    ...
    return {
        ...
        "core_memory": {
            "tokens": core_stats["tokens"],
            "max_tokens": core_stats["max_tokens"],
            "over_budget": core_stats["tokens"] > core_stats["max_tokens"],
...
def _check_runtime() -> dict:
    ...
    return {"error_count_24h": len(errors)}
...
def run_checkup(user_id: int = 0) -> dict:
    ...
    for key, fn, args in [
        ("prompt_size", _check_prompt_size, ()),
