...
def _run_migrations(conn: sqlite3.Connection) -> None:
    ...
    if attention_table is not None:
        ...
        retired = conn.execute(
            ...
            "WHERE entry_kind IN ('attention', 'free_time') "
            "AND status IN ('pending', 'running', 'ready', 'deferred')",
            (retired_at,),
        ).rowcount
        conn.execute("DELETE FROM heartbeat_schedules")
        conn.execute("DROP INDEX IF EXISTS idx_attention_facts_status")
        conn.execute("DROP TABLE attention_facts")
        logger.info(
            "Migrated to unified Free Time; retired %d unfinished autonomous run(s)",
            retired,
        )
